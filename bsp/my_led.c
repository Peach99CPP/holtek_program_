//
// Created by peachRJ on 2021/4/15.
//

#include "my_led.h"
#include "timer_it.h"
#include "bsp_delay.h"
#include "oledfont.h"
u16 BACK_COLOR = WHITE;

/******************************************************************************
      函数说明：LCD串行数据写入函数
      入口数据：dat  要写入的串行数据
      返回值：  无
******************************************************************************/
void LCD_Writ_Bus(int id, u8 dat) {
    u8 i;
    if (id == 1) {
        A_OLED_CS_RESET();
        for (i = 0; i < 8; i++
                ) {
            A_OLED_SCLK_RESET();
            if (dat & 0x80)
                A_OLED_SDA_SET();
            else
                A_OLED_SDA_RESET();
            A_OLED_SCLK_SET();
            dat <<= 1;
        }
        A_OLED_CS_SET();
    } else if (id == 2) {
        B_OLED_CS_RESET();
        for (i = 0; i < 8; i++
                ) {
            B_OLED_SCLK_RESET();
            if (dat & 0x80)
                B_OLED_SDA_SET();
            else
                B_OLED_SDA_RESET();
            B_OLED_SCLK_SET();
            dat <<= 1;
        }
        B_OLED_CS_SET();
    }
}


/******************************************************************************
      函数说明：LCD写入数据
      入口数据：dat 写入的数据
      返回值：  无
******************************************************************************/
void LCD_WR_DATA8(int id, u8 dat) {
    if (id == 1)
        A_OLED_DC_SET();
    else
        B_OLED_DC_SET();
    LCD_Writ_Bus(id, dat);
}


/******************************************************************************
      函数说明：LCD写入数据
      入口数据：dat 写入的数据
      返回值：  无
******************************************************************************/
void LCD_WR_DATA(int id, u16 dat) {
    if (id == 1)
        A_OLED_DC_SET();
    else if(id==2)
        B_OLED_DC_SET();
    else return;
    LCD_Writ_Bus(id, dat >> 8);//高8位
    LCD_Writ_Bus(id, dat);//低8位
}


/******************************************************************************
      函数说明：LCD写入命令
      入口数据：dat 写入的命令
      返回值：  无
******************************************************************************/
void LCD_WR_REG(int id, u8 dat) {
    if (id == 1)
        A_OLED_DC_RESET();
    else
        B_OLED_DC_RESET();
    LCD_Writ_Bus(id, dat);
}


/******************************************************************************
      函数说明：设置起始和结束地址
      入口数据：x1,x2 设置列的起始和结束地址
                y1,y2 设置行的起始和结束地址
      返回值：  无
******************************************************************************/
void LCD_Address_Set(int id, u16 x1, u16 y1, u16 x2, u16 y2) {
    if (USE_HORIZONTAL == 0) {
        LCD_WR_REG(id, 0x2a);//列地址设置
        LCD_WR_DATA(id, x1 + 2);
        LCD_WR_DATA(id, x2 + 2);
        LCD_WR_REG(id, 0x2b);//行地址设置
        LCD_WR_DATA(id, y1 + 1);
        LCD_WR_DATA(id, y2 + 1);
        LCD_WR_REG(id, 0x2c);//储存器写
    } else if (USE_HORIZONTAL == 1) {
        LCD_WR_REG(id, 0x2a);//列地址设置
        LCD_WR_DATA(id, x1 + 2);
        LCD_WR_DATA(id, x2 + 2);
        LCD_WR_REG(id, 0x2b);//行地址设置
        LCD_WR_DATA(id, y1 + 1);
        LCD_WR_DATA(id, y2 + 1);
        LCD_WR_REG(id, 0x2c);//储存器写
    } else if (USE_HORIZONTAL == 2) {
        LCD_WR_REG(id, 0x2a);//列地址设置
        LCD_WR_DATA(id, x1 + 1);
        LCD_WR_DATA(id, x2 + 1);
        LCD_WR_REG(id, 0x2b);//行地址设置
        LCD_WR_DATA(id, y1 + 2);
        LCD_WR_DATA(id, y2 + 2);
        LCD_WR_REG(id, 0x2c);//储存器写
    } else {
        LCD_WR_REG(id, 0x2a);//列地址设置
        LCD_WR_DATA(id, x1 + 1);
        LCD_WR_DATA(id, x2 + 1);
        LCD_WR_REG(id, 0x2b);//行地址设置
        LCD_WR_DATA(id, y1 + 2);
        LCD_WR_DATA(id, y2 + 2);
        LCD_WR_REG(id, 0x2c);//储存器写
    }
}


/******************************************************************************
      函数说明：LCD初始化函数
      入口数据：无
      返回值：  无
******************************************************************************/
void Lcd_Init(int id) {
    if (id == 1) {
        A_OLED_RES_RESET();
        delay_ms(20);
        A_OLED_RES_SET();
        delay_ms(20);
        A_OLED_BL_ON();
    } else if (id == 2) {
        B_OLED_RES_RESET();
        delay_ms(20);
        B_OLED_RES_SET();
        delay_ms(20);
        B_OLED_BL_ON();
    }

//************* Start Initial Sequence **********//
    LCD_WR_REG(id, 0x11); //Sleep out
    delay_ms(120);              //Delay 120ms 
//------------------------------------ST7735S Frame Rate-----------------------------------------// 
    LCD_WR_REG(id, 0xB1);
    LCD_WR_DATA8(id, 0x05);
    LCD_WR_DATA8(id, 0x3C);
    LCD_WR_DATA8(id, 0x3C);
    LCD_WR_REG(id, 0xB2);
    LCD_WR_DATA8(id, 0x05);
    LCD_WR_DATA8(id, 0x3C);
    LCD_WR_DATA8(id, 0x3C);
    LCD_WR_REG(id, 0xB3);
    LCD_WR_DATA8(id, 0x05);
    LCD_WR_DATA8(id, 0x3C);
    LCD_WR_DATA8(id, 0x3C);
    LCD_WR_DATA8(id, 0x05);
    LCD_WR_DATA8(id, 0x3C);
    LCD_WR_DATA8(id, 0x3C);
//------------------------------------End ST7735S Frame Rate---------------------------------// 
    LCD_WR_REG(id, 0xB4); //Dot inversion
    LCD_WR_DATA8(id, 0x03);
//------------------------------------ST7735S Power Sequence---------------------------------// 
    LCD_WR_REG(id, 0xC0);
    LCD_WR_DATA8(id, 0x28);
    LCD_WR_DATA8(id, 0x08);
    LCD_WR_DATA8(id, 0x04);
    LCD_WR_REG(id, 0xC1);
    LCD_WR_DATA8(id, 0XC0);
    LCD_WR_REG(id, 0xC2);
    LCD_WR_DATA8(id, 0x0D);
    LCD_WR_DATA8(id, 0x00);
    LCD_WR_REG(id, 0xC3);
    LCD_WR_DATA8(id, 0x8D);
    LCD_WR_DATA8(id, 0x2A);
    LCD_WR_REG(id, 0xC4);
    LCD_WR_DATA8(id, 0x8D);
    LCD_WR_DATA8(id, 0xEE);
//---------------------------------End ST7735S Power Sequence-------------------------------------// 
    LCD_WR_REG(id, 0xC5); //VCOM
    LCD_WR_DATA8(id, 0x1A);
    LCD_WR_REG(id, 0x36); //MX, MY, RGB mode
    if (USE_HORIZONTAL == 0)
        LCD_WR_DATA8(id, 0x00);
    else if (USE_HORIZONTAL == 1)
        LCD_WR_DATA8(id, 0xC0);
    else if (USE_HORIZONTAL == 2)
        LCD_WR_DATA8(id, 0x70);
    else
        LCD_WR_DATA8(id, 0xA0);
//------------------------------------ST7735S Gamma Sequence---------------------------------// 
    LCD_WR_REG(id, 0xE0);
    LCD_WR_DATA8(id, 0x04);
    LCD_WR_DATA8(id, 0x22);
    LCD_WR_DATA8(id, 0x07);
    LCD_WR_DATA8(id, 0x0A);
    LCD_WR_DATA8(id, 0x2E);
    LCD_WR_DATA8(id, 0x30);
    LCD_WR_DATA8(id, 0x25);
    LCD_WR_DATA8(id, 0x2A);
    LCD_WR_DATA8(id, 0x28);
    LCD_WR_DATA8(id, 0x26);
    LCD_WR_DATA8(id, 0x2E);
    LCD_WR_DATA8(id, 0x3A);
    LCD_WR_DATA8(id, 0x00);
    LCD_WR_DATA8(id, 0x01);
    LCD_WR_DATA8(id, 0x03);
    LCD_WR_DATA8(id, 0x13);
    LCD_WR_REG(id, 0xE1);
    LCD_WR_DATA8(id, 0x04);
    LCD_WR_DATA8(id, 0x16);
    LCD_WR_DATA8(id, 0x06);
    LCD_WR_DATA8(id, 0x0D);
    LCD_WR_DATA8(id, 0x2D);
    LCD_WR_DATA8(id, 0x26);
    LCD_WR_DATA8(id, 0x23);
    LCD_WR_DATA8(id, 0x27);
    LCD_WR_DATA8(id, 0x27);
    LCD_WR_DATA8(id, 0x25);
    LCD_WR_DATA8(id, 0x2D);
    LCD_WR_DATA8(id, 0x3B);
    LCD_WR_DATA8(id, 0x00);
    LCD_WR_DATA8(id, 0x01);
    LCD_WR_DATA8(id, 0x04);
    LCD_WR_DATA8(id, 0x13);
//------------------------------------End ST7735S Gamma Sequence-----------------------------// 
    LCD_WR_REG(id, 0x3A); //65k mode
    LCD_WR_DATA8(id, 0x05);
    LCD_WR_REG(id, 0x29); //Display on
}


/******************************************************************************
      函数说明：LCD清屏函数
      入口数据：无
      返回值：  无
******************************************************************************/
void LCD_Clear(int id, u16 Color) {
    u16 i, j;
    LCD_Address_Set(id, 0, 0, LCD_W - 1, LCD_H - 1);
    for (i = 0; i < LCD_W; i++
            ) {
        for (j = 0; j < LCD_H; j++
                ) {
            LCD_WR_DATA(id, Color);
        }
    }
}


/******************************************************************************
      函数说明：LCD显示汉字
      入口数据：x,y   起始坐标
                index 汉字的序号
                size  字号
      返回值：  无
******************************************************************************/
void LCD_ShowChinese(int id, u16 x, u16 y, u8 index, u8 size, u16 color) {  //字体已经写死了
    u8 i, j;
    u8 *temp, size1;//指针
    unsigned char *Hzk16,*Hzk32;
    if (size == 16) {
        temp =Hzk16;
    }//选择字号
    if (size == 32) {
        temp = Hzk32;
    }
    LCD_Address_Set(id, x, y, x + size - 1, y + size - 1); //设置一个汉字的区域
    size1 = size * size / 8;//一个汉字所占的字节
    temp += index * size1;//写入的起始位置，从数组哪个位置开始读，以确是哪一个字
    for (j = 0; j < size1; j++
            ) {
        for (i = 0; i < 8; i++
                ) {
            if ((*temp & (1 << i)) != 0)//从数据的低位开始读
            {
                LCD_WR_DATA(id, color);//点亮
            } else {
                LCD_WR_DATA(id, BACK_COLOR);//不点亮
            }
        }
        temp++;
    }
}

/******************************************************************************
      函数说明：LCD显示汉字
      入口数据：x,y   起始坐标
      返回值：  无
******************************************************************************/
void LCD_showTest(int id, u16 x, u16 y, u8 index, u16 color) {
    u8 i, j;
    u8 *temp, size1,*My_num;//指针
    temp = My_num;//指定写入的
    LCD_Address_Set(id, x, y, x + 32 - 1, y + 32 - 1); //设置一个汉字的区域
    size1 = 32 * 32 / 8;
    temp += index * size1;//得到起始位置
    for (j = 0; j < size1; j++
            ) {
        for (i = 0; i < 8; i++
                ) {
            if ((*temp & (1 << i)) != 0)//从数据的低位开始读
            {
                LCD_WR_DATA(id, color);//点亮
            } else {
                LCD_WR_DATA(id, BACK_COLOR);//不点亮
            }
        }
        temp++;
    }

}

/**************************
函数功能 产生一个32*32的数字格子
 每两个为一个数字 故写一个数字需要调用两次
index  索引 用于在数组中找到对应数字的码数
**************************/
void LCD_BigNum(int id, u16 x, u16 y, u8 num, u16 color) {
    switch (num) {
        case 0:
            LCD_showTest(id, x, y, 0, color);
            LCD_showTest(id, x, y + 32, 1, color);
            break;
        case 1:
            LCD_showTest(id, x, y, 2, color);
            LCD_showTest(id, x, y + 32, 3, color);
            break;
        case 2:
            LCD_showTest(id, x, y, 4, color);
            LCD_showTest(id, x, y + 32, 5, color);
            break;
        case 3:
            LCD_showTest(id, x, y, 6, color);
            LCD_showTest(id, x, y + 32, 7, color);
            break;
        case 4:
            LCD_showTest(id, x, y, 8, color);
            LCD_showTest(id, x, y + 32, 9, color);
            break;
        case 5:
            LCD_showTest(id, x, y, 10, color);
            LCD_showTest(id, x, y + 32, 11, color);
            break;
        case 6:
            LCD_showTest(id, x, y, 12, color);
            LCD_showTest(id, x, y + 32, 13, color);
            break;
        case 7:
            LCD_showTest(id, x, y, 14, color);
            LCD_showTest(id, x, y + 32, 15, color);
            break;
        case 8:
            LCD_showTest(id, x, y, 16, color);
            LCD_showTest(id, x, y + 32, 17, color);
            break;
        case 9:
            LCD_showTest(id, x, y, 18, color);
            LCD_showTest(id, x, y + 32, 19, color);
            break;
        default:;//还没想好 可以调用LED频闪反馈出错
    }

}

/******************************************************************************
      函数说明：LCD显示大数字
      入口数据：x,y   起始坐标
			封装 LCD_showTest函数，一个数字调用2次
      返回值：  无
******************************************************************************/
void LCD_DrawPoint(int id, u16 x, u16 y, u16 color) {
    LCD_Address_Set(id, x, y, x, y);//设置光标位置
    LCD_WR_DATA(id, color);
}


/******************************************************************************
      函数说明：LCD画一个大的点
      入口数据：x,y   起始坐标
      返回值：  无
******************************************************************************/
void LCD_DrawPoint_big(int id, u16 x, u16 y, u16 color) {
    LCD_Fill(id, x - 1, y - 1, x + 1, y + 1, color);
}


/******************************************************************************
      函数说明：在指定区域填充颜色
      入口数据：xsta,ysta   起始坐标
                xend,yend   终止坐标
      返回值：  无
******************************************************************************/
void LCD_Fill(int id, u16 xsta, u16 ysta, u16 xend, u16 yend, u16 color) {
    u16 i, j;
    LCD_Address_Set(id, xsta, ysta, xend, yend);      //设置光标位置
    for (i = ysta; i <= yend; i++
            ) {
        for (j = xsta; j <= xend; j++)
            LCD_WR_DATA(id, color);//设置光标位置
    }
}


/******************************************************************************
      函数说明：画线
      入口数据：x1,y1   起始坐标
                x2,y2   终止坐标
      返回值：  无
******************************************************************************/
void LCD_DrawLine(int id, u16 x1, u16 y1, u16 x2, u16 y2, u16 color) {
    u16 t;
    int xerr = 0, yerr = 0, delta_x, delta_y, distance;
    int incx, incy, uRow, uCol;
    delta_x = x2 - x1; //计算坐标增量
    delta_y = y2 - y1;
    uRow = x1;//画线起点坐标
    uCol = y1;
    if (delta_x > 0)
        incx = 1; //设置单步方向
    else if (delta_x == 0)
        incx = 0;//垂直线
    else {
        incx = -1;
        delta_x = -delta_x;
    }
    if (delta_y > 0)
        incy = 1;
    else if (delta_y == 0)
        incy = 0;//水平线
    else {
        incy = -1;
        delta_y = -delta_x;
    }
    if (delta_x > delta_y)
        distance = delta_x; //选取基本增量坐标轴
    else
        distance = delta_y;
    for (t = 0; t < distance + 1; t++
            ) {
        LCD_DrawPoint(id, uRow, uCol, color);//画点
        xerr += delta_x;
        yerr += delta_y;
        if (xerr > distance) {
            xerr -= distance;
            uRow += incx;
        }
        if (yerr > distance) {
            yerr -= distance;
            uCol += incy;
        }
    }
}


/******************************************************************************
      函数说明：画矩形
      入口数据：x1,y1   起始坐标
                x2,y2   终止坐标
      返回值：  无
******************************************************************************/
void LCD_DrawRectangle(int id, u16 x1, u16 y1, u16 x2, u16 y2, u16 color) {
    LCD_DrawLine(id, x1, y1, x2, y1, color);
    LCD_DrawLine(id, x1, y1, x1, y2, color);
    LCD_DrawLine(id, x1, y2, x2, y2, color);
    LCD_DrawLine(id, x2, y1, x2, y2, color);
}


/******************************************************************************
      函数说明：画圆
      入口数据：x0,y0   圆心坐标
                r       半径
      返回值：  无
******************************************************************************/
void Draw_Circle(int id, u16 x0, u16 y0, u8 r, u16 color) {
    int a, b;
    int di;
    a = 0;
    b = r;
    while (a <= b) {
        LCD_DrawPoint(id, x0 - b, y0 - a, color);             //3
        LCD_DrawPoint(id, x0 + b, y0 - a, color);             //0
        LCD_DrawPoint(id, x0 - a, y0 + b, color);             //1
        LCD_DrawPoint(id, x0 - a, y0 - b, color);             //2
        LCD_DrawPoint(id, x0 + b, y0 + a, color);             //4
        LCD_DrawPoint(id, x0 + a, y0 - b, color);             //5
        LCD_DrawPoint(id, x0 + a, y0 + b, color);             //6
        LCD_DrawPoint(id, x0 - b, y0 + a, color);             //7
        a++;
        if ((a * a + b * b) > (r * r))//判断要画的点是否过远
        {
            b--;
        }
    }
}


/******************************************************************************
      函数说明：显示字符
      入口数据：x,y    起点坐标
                num    要显示的字符
                mode   1叠加方式  0非叠加方式
      返回值：  无
******************************************************************************/
void LCD_ShowChar(int id, u16 x, u16 y, u8 num, u8 mode, u16 color) {
    u8 temp;
    u8 pos, t;
    u16 x0 = x;
    u8 * asc2_1608;
    if (x > LCD_W - 16 || y > LCD_H - 16)
        return;        //设置窗口
    num = num - ' ';//得到偏移后的值
    LCD_Address_Set(id, x, y, x + 8 - 1, y + 16 - 1);      //设置光标位置
    if (!mode) //非叠加方式
    {
        for (pos = 0; pos < 16; pos++
                ) {
            temp = asc2_1608[(u16) num * 16 + pos];         //调用1608字体
            for (t = 0; t < 8; t++
                    ) {
                if (temp & 0x01)
                    LCD_WR_DATA(id, color);
                else
                    LCD_WR_DATA(id, BACK_COLOR);
                temp >>= 1;
                x++;
            }
            x = x0;
            y++;
        }
    } else//叠加方式
    {
        for (pos = 0; pos < 16; pos++
                ) {
            temp = asc2_1608[(u16) num * 16 + pos];         //调用1608字体
            for (t = 0; t < 8; t++
                    ) {
                if (temp & 0x01)
                    LCD_DrawPoint(id, x + t, y + pos, color);//画一个点
                temp >>= 1;
            }
        }
    }
}


/******************************************************************************
      函数说明：显示字符串
      入口数据：x,y    起点坐标
                *p     字符串起始地址
      返回值：  无
******************************************************************************/
void LCD_ShowString(int id, u16 x, u16 y, const u8 *p, u16 color) {
    while (*p != '\0') {
        if (x > LCD_W - 16) {
            x = 0;
            y += 16;
        }
        if (y > LCD_H - 16) {
            y = x = 0;
            LCD_Clear(id, RED);
        }
        LCD_ShowChar(id, x, y, *p, 0, color);
        x += 8;
        p++;
    }
}


/******************************************************************************
      函数说明：显示数字
      入口数据：m底数，n指数
      返回值：  无
******************************************************************************/
u32 mypow(int id, u8 m, u8 n) {
    u32 result = 1;
    while (n--)
        result *= m;
    return result;
}


/******************************************************************************
      函数说明：显示数字
      入口数据：x,y    起点坐标
                num    要显示的数字
                len    要显示的数字个数
      返回值：  无
******************************************************************************/
void LCD_ShowNum(int id, u16 x, u16 y, u16 num, u8 len, u16 color) {
    u8 t, temp;
    u8 enshow = 0;
    for (t = 0; t < len; t++
            ) {
        temp = (num / mypow(id, 10, len - t - 1)) % 10;//通过取模运算把数字的每一位算出来
        if (enshow == 0 && t < (len - 1)) {
            if (temp == 0) {
                LCD_ShowChar(id, x + 8 * t, y, ' ', 0, color);
                continue;
            } else
                enshow = 1;

        }
        LCD_ShowChar(id, x + 8 * t, y, temp + 48, 0, color);
    }
}


/******************************************************************************
      函数说明：显示小数
      入口数据：x,y    起点坐标
                num    要显示的小数
                len    要显示的数字个数
      返回值：  无
******************************************************************************/
void LCD_ShowNum1(int id, u16 x, u16 y, float num, u8 len, u16 color) {
    u8 t, temp;
    u8 enshow = 0;
    u16 num1;
    num1 = num * 100;
    for (t = 0; t < len; t++
            ) {
        temp = (num1 / mypow(id, 10, len - t - 1)) % 10;
        if (t == (len - 2)) {
            LCD_ShowChar(id, x + 8 * (len - 2), y, '.', 0, color);
            t++;
            len += 1;
        }
        LCD_ShowChar(id, x + 8 * t, y, temp + 48, 0, color);
    }
}


/******************************************************************************
      函数说明：显示40x40图片
      入口数据：x,y    起点坐标
      返回值：  无
******************************************************************************/
void LCD_ShowPicture(int id, u16 x1, u16 y1, u16 x2, u16 y2) {
    int i;
    u8 * gImage_scren_code_img=gImage_a1_l;
    LCD_Address_Set(id, x1, y1, x2, y2);
    for (i = 0; i < 1600; i++
            ) {
        LCD_WR_DATA8(id, gImage_scren_code_img[i * 2 + 1]);
        LCD_WR_DATA8(id, gImage_scren_code_img[i * 2]);
    }
}
void LCD_ShowMyChinese(int id,u8 index,u16 x,u16 y,const u8 * ptr,u16 x_size,u16 y_size,u16 color)
{
    if(ptr==NULL ) return ;
    LCD_Address_Set(id,x,y,x+x_size-1,y+y_size-1);
    u16 i,bytes_num,j;
    bytes_num=x_size*y_size/8;
    ptr+=index*bytes_num;
    for(i=0;i<bytes_num;++i)
    {
        for (j = 0; j < 8; j++
                ) {
            if ((*ptr & (1 << j)) != 0)//从数据的低位开始读
            {
                LCD_WR_DATA(id, color);//点亮
            } else {
                LCD_WR_DATA(id, BACK_COLOR);//不点亮
            }
        }
        ptr++;
    }
}
void LCD_ShowMyPicture(int id,u16 x,u16 y,const u8 * ptr,u16 x_size,u16 y_size)
{

    LCD_Address_Set(1,x,y,x+x_size-1,y+y_size-1);
    u16 bits_num=x_size*y_size;
    u16 i;
    for(i=0;i<bits_num;++i)
    {
        LCD_WR_DATA8(id, ptr[i * 2 + 1]);
        LCD_WR_DATA8(id, ptr[i * 2]);
    }
}
void LCD_Show_Expressions(int id)
{
#define x_start 32
#define y_start 32
    LCD_Clear(1,WHITE);
    LCD_Clear(2,WHITE);
    if(id==1)
    {
        LCD_ShowMyPicture(1,x_start,y_start,gImage_a1_l,64,79);
        LCD_ShowMyPicture(2,x_start,y_start,gImage_a1_r,64,79);
    }
    else if (id == 2)
    {
        LCD_ShowMyPicture(1, x_start, y_start, gImage_a1_l, 64, 79);
        LCD_ShowMyPicture(2, x_start, y_start, gImage_a1_r, 64, 79);
    }
    else if (id == 3)
    {
        LCD_ShowMyPicture(1, x_start, y_start, gImage_a3_l, 64, 74);
        LCD_ShowMyPicture(2, x_start, y_start, gImage_a3_r, 64, 71);
    }
}

void Show_Chinese(int number)
{
    LCD_Clear(1, WHITE);
    LCD_Clear(2, WHITE);
    delay_ms(50);
    LCD_ShowMyChinese(1,2*number-1,32,32,chinese_font,64,64,BROWN);
    LCD_ShowMyChinese(2,2*number-2,32,32,chinese_font,64,64,BROWN);

}
void LED_ShowMY_Num(int id,int num)
{
    LCD_Clear(id,WHITE);
    LCD_ShowMyChinese(id,num,32,32,My_Num,64,64,RED);
}